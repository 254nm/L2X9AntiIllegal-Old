package me.txmc.unbreakablepatch.jagent;
        ...
        public byte[] transform(ClassLoader classLoader, String className, Class<?> klass, ProtectionDomain protectionDomain, byte[] currentBytes) throws IllegalClassFormatException {
            ...
            try {
                if (className.equals(getName(ItemStack.class))) {
                    ...
                    defaultCon.insertAfter("org.bukkit.Bukkit.getServer().getPluginManager().callEvent(new me.txmc.unbreakablepatch.events.ItemStackCreateEvent(this, me.txmc.unbreakablepatch.events.ItemStackCreateEvent.ItemStackCreateReason.DEFAULT));");
                    CtConstructor nbtCon = cc.getDeclaredConstructor(new CtClass[]{classPool.get(NBTTagCompound.class.getName())});
                    nbtCon.insertAfter("org.bukkit.Bukkit.getServer().getPluginManager().callEvent(new me.txmc.unbreakablepatch.events.ItemStackCreateEvent(this, me.txmc.unbreakablepatch.events.ItemStackCreateEvent.ItemStackCreateReason.NBT));");
                    cc.detach();
